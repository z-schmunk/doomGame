...
ENEMY_DAMAGE        = 10
ENEMY_ATTACK_RATE   = 1.5        # seconds between shots (slower since bullets now travel)
ENEMY_DEATH_DISPLAY = 0.7
BULLET_SPEED        = 0.18
ENEMY_BULLET_SPEED  = 0.25       # slightly slower than player bullets so it's fair
ENEMY_BULLET_RADIUS = 0.25       # how close an enemy bullet must get to hit player
...
def generate_map():
    ...
    placed = []
    for _ in range(30):             # 30 attempts to place 10 rooms
...
def find_spawn(min_dist_from=None, min_dist=8.0):
    # Choose a truly random free tile instead of always using room centers.
    for _ in range(2000):
        x = random.randint(1, MAP_W-2)
        y = random.randint(1, MAP_H-2)
        if WORLD_MAP[y][x] != 0:
        ...
        if position_free(px, py):
            return px, py
    # Fall back to any open tile if random sampling fails
...
def spawn_enemies(count, avoid_x, avoid_y):
    ...
    for _ in range(count * 80):      # generous attempt budget
...
class Player:
    ...
    def take_hit(self, amount):
        ...
        if self.dmg_cooldown <= 0:
            ...
            self.dmg_cooldown = 1.0    # 1.0 s invincibility
            ...
            if dist > ENEMY_ATTACK_RANGE:
                ...
                if blocked:
                    ...
                    if enemy.dmg_timer <= 0:
                        # Fire a bullet toward the player instead of instant damage
                        angle_to_player = math.atan2(
                            player.y - enemy.y,
                            player.x - enemy.x
                        )
                        # Small random spread so enemies aren't perfect aimers
                        spread = random.uniform(-0.05, 0.05)   # pretty accurate

                        enemy_bullets.append(
                            EnemyBullet(enemy.x, enemy.y, angle_to_player + spread)
                        )
...
def update_and_draw_enemy_bullets(screen, player, z_buffer):
    ...
    # Render – same projection math as player bullets, different colour
    for b in enemy_bullets:
        ...
        screen_y = HALF_H + vertical_shift
        # Red/orange colour so player can clearly distinguish enemy fire
...
def main():
    ...
    while running:
        ...
        update_and_draw_enemy_bullets(screen, player, z_buffer)      # enemy bullets
